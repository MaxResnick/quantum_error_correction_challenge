...
def test_run_benchmark_end_to_end() -> None:
    ...
    grid = tiny_grid()
    result = run_benchmark(build_decoder_fn=_mwpm, grid=grid, shots_per_point=500, seed=42)
...
def test_deterministic_with_same_seed() -> None:
    ...
    grid = tiny_grid()[:1]
    r1 = run_benchmark(_mwpm, grid, shots_per_point=200, seed=99)
    r2 = run_benchmark(_mwpm, grid, shots_per_point=200, seed=99)
...
def test_all_zeros_decoder_worse_than_mwpm() -> None:
    ...
    grid = tiny_grid()
    mwpm_result = run_benchmark(_mwpm, grid, shots_per_point=1000, seed=42)
    zero_result = run_benchmark(
        build_decoder_fn=lambda point: ZeroDecoder(),
        grid=grid, shots_per_point=1000, seed=42,
    )
